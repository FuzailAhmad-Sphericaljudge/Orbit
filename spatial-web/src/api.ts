...
export const orbitApi = {
  ...
  addParticipant: (incidentId: string, payload: { agora_uid: string; display_name: string; role: string; language: string }) => request<{ id: string }>(`/api/incidents/${incidentId}/participants`, { method: "POST", body: JSON.stringify(payload) }),
  generateBriefing: (incidentId: string, payload: { audience: "commander" | "engineering" | "support" | "executive"; speak: boolean; voice_session_id?: string }) => request<{ id: string; spoken: boolean }>(`/api/incidents/${incidentId}/briefings`, { method: "POST", body: JSON.stringify(payload) }),
  addTranscript: (incidentId: string, payload: { speaker_name: string; speaker_role: string; text: string; language: string; is_final: boolean }) => request<{ id: string }>(`/api/incidents/${incidentId}/transcripts`, { method: "POST", body: JSON.stringify(payload) }),
  runInvestigation: (incidentId: string) => request<{ unknowns_open: number }>(`/api/incidents/${incidentId}/investigation/run`, { method: "POST", body: JSON.stringify({}) }),
  commandCenter: (incidentId: string) => request<CommandCenterSnapshot>(`/api/incidents/${incidentId}/command-center`),
  addEvidence: (incidentId: string, payload: { claim: string; classification: "confirmed_fact" | "hypothesis" | "decision" | "action"; confidence: number; source: string }) => request<{ id: string }>(`/api/incidents/${incidentId}/evidence`, { method: "POST", body: JSON.stringify(payload) }),
  prepareTool: (incidentId: string, payload: { provider: "slack" | "jira" | "pagerduty" | "monitoring"; operation: string; risk_level: "low" | "medium" | "high"; payload: Record<string, unknown>; rationale: string; idempotency_key: string }) => request<{ id: string; approval_id?: string | null }>(`/api/incidents/${incidentId}/tools/prepare`, { method: "POST", body: JSON.stringify(payload) }),
  executeTool: (incidentId: string, executionId: string) => request<{ id: string; status: string }>(`/api/incidents/${incidentId}/tools/${executionId}/execute`, { method: "POST" }),
  decideApproval: (incidentId: string, approvalId: string, status: "approved" | "rejected") => request<{ id: string; status: string }>(`/api/incidents/${incidentId}/approvals/${approvalId}`, { method: "POST", body: JSON.stringify({ status }) }),
